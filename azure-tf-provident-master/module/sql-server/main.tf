...
resource "azurerm_log_analytics_workspace" "createlog" {
...
data "azurerm_log_analytics_workspace" "logws" {
  count               = azurerm_log_analytics_workspace.createlog.name != null ? 1 : 0
  name                = azurerm_log_analytics_workspace.createlog.name
  resource_group_name = local.resource_group_name
}
...
resource "azurerm_sql_database" "db" {
  count                            = length(var.database_name) > 0 ? length(var.database_name) : 0
  ...
  tags                             = merge({ "Name" = format("%s-primary", var.database_name[count.index]) }, var.tags, )
...
resource "azurerm_monitor_diagnostic_setting" "extaudit" {
  ...
  name                       = lower("extaudit-${azurerm_sql_database.db[count.index].name}-diag")
